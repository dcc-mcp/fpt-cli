...
### Dependency updates and workspace-hack

`workspace-hack/Cargo.toml` is generated by [cargo hakari](https://docs.rs/cargo-hakari) from the
dependency graph, and `just ci` fails on its `hakari-check` step while it is stale. A dependency
bump that changes the feature graph must therefore regenerate it on the same branch:

```bash
vx just hakari-generate   # cargo hakari generate + cargo hakari manage-deps -y
vx just hakari-check      # verify before pushing
```

Bot branches are handled by `.github/workflows/hakari-autofix.yml`, which detects the drift on
renovate and dependabot pull requests and pushes a `chore(hakari): regenerate workspace-hack`
commit. It only pushes when the PAT `HAKARI_AUTOFIX_TOKEN` is configured, because a push made with
`GITHUB_TOKEN` does not start a new workflow run: that would move the PR head to a commit no CI run
ever validates, i.e. it hides the failing check instead of fixing it. The token has to be stored
twice — as an **Actions secret** (read by renovate and `workflow_dispatch` runs) and as a
**Dependabot secret** (read by Dependabot runs). Without it the workflow reports the drift in its
job summary and leaves the branch untouched, so the drift still fails `Quality Checks`.

Renovate's `postUpgradeTasks.commands` is not used because `allowedCommands` is a global-only
option that the hosted Mend Renovate App cannot set.

The pinned `cargo-hakari` version lives in `CARGO_HAKARI_VERSION` in both workflows and must be
bumped deliberately, so a tool release cannot change the result of `hakari-check`.

That workflow can also be started manually with `workflow_dispatch`. It requires an explicit `ref`
and holds `contents: write`, so it will push a regenerated `workspace-hack` straight to that ref:
pin it to a branch you intend to change, not to `main`.
...
# Webhooks
fpt webhook hooks-list --site ...
fpt webhook hook-create --input '{"event_type":"shotgrid.entity.Shot.change","url":"https://example.com/hook"}' --site ...
fpt webhook hook-read 9f1c2b7e --site ...
fpt webhook hook-update 9f1c2b7e --input '{"url":"https://example.com/updated"}' --site ...
fpt webhook hook-delete 9f1c2b7e --site ...
fpt webhook hook-test 9f1c2b7e --site ...
fpt webhook deliveries-list 9f1c2b7e --input '{"status":"failed"}' --site ...
fpt webhook delivery-read 4a2d9f10 --site ...
fpt webhook delivery-update 4a2d9f10 --input '{"acknowledgement":"reviewed"}' --site ...
fpt webhook delivery-redeliver 4a2d9f10 --site ...

# Subscription seats
fpt subscription user-list --site ...
