...
commit. Dependabot-triggered runs receive a read-only `GITHUB_TOKEN`, so pushing from those runs
needs a PAT stored as the Dependabot secret `HAKARI_AUTOFIX_TOKEN`; without it the workflow reports
the drift and leaves the commit to a human. Renovate's `postUpgradeTasks.commands` is not used
because `allowedCommands` is a global-only option that the hosted Mend Renovate App cannot set.
